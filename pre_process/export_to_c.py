...
#載入訓練好的模型與樣本
model = tf.keras.models.load_model(BASE_DIR / 'best_model.h5') #載入訓練好的模型
X_test_sub = np.load(BASE_DIR / 'X_test_sample.npy') #載入10筆測試樣本 (128, 2)
y_test_sub = np.load(BASE_DIR / 'y_test_sample.npy') #載入10筆測試標籤 (one-hot格式)

def export_to_header(model, X_sample, y_sample, filename=BASE_DIR / 'model_params.h'):
    with open(filename, 'w') as f:
        #寫入C header格式，避免變數重複定義
